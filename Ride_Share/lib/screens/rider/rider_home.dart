...
class _RiderHomeScreenState extends State<RiderHomeScreen> {
  ...
  Future<void> _loadBookedRides() async {
    ...
    try {
      ...
      for (var map in rideMaps) {
        try {
          parsedRides.add(Ride.fromMap(map));
        } catch (e) {
          print('Error parsing ride: $e');
        }
      }
      if(mounted) setState(() => _bookedRides = parsedRides);
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to load booked rides: $e'), backgroundColor: AppColors.errorColor));
    } finally {
      if(mounted) setState(() => _isLoadingBookedRides = false);
    }
  }
  ...
  Future<void> _cancelBooking(String rideId, String? reason) async {
    ...
    try {
      ...
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Ride booking cancelled successfully!')));
      _loadBookedRides();
    } catch (e) {
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to cancel booking: $e')));
    } finally {
      if (mounted) setState(() => _isLoadingBookedRides = false);
    }
  }
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: List.generate(5, (index) {
                      return IconButton(
                        icon: Icon(index < (selectedRating ?? 0) ? Icons.star : Icons.star_border, color: Colors.amber, size: 36),
            ...
            CustomButton(
              ...
              onPressed: () async {
                if (selectedRating != null) {
                  ...
                  if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Please select a rating.')));
                }
              },
  ...
  Future<void> _rateDriver(String rideId, int rating) async {
    ...
    try {
      ...
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Driver rated successfully!')));
      _loadBookedRides();
    } catch (e) {
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Failed to rate driver: $e')));
    } finally {
      if (mounted) setState(() => _isLoadingBookedRides = false);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(icon: const Icon(Icons.refresh), onPressed: _loadBookedRides),
        ...
        child: Column(
          ...
          children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text('Welcome, Rider ${appAuthProvider.appUser?.name ?? ''}!', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
            ...
            CustomButton(
              text: 'Find Rides',
              onPressed: () async {
                await Navigator.push(context, MaterialPageRoute(builder: (context) => const FindRidesScreen()));
                _loadBookedRides();
              },
              color: AppColors.primaryColor,
            ),
            SizedBox(height: screenSize.height * 0.02),
            const Text('Your Booked Rides History:', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
            SizedBox(height: screenSize.height * 0.01),
            _isLoadingBookedRides
                ? const Center(child: LoadingIndicator())
                : Expanded(
                    child: _bookedRides.isEmpty
                        ? Center(child: Text('No rides booked yet. Find your first ride!', style: TextStyle(color: AppColors.hintColor)))
                        : ListView.builder(
                            ...
                            itemBuilder: (context, index) {
                              ...
                              final currentUserBooking = ride.passengers.firstWhereOrNull((p) => p.userId == currentUserMongoId);
                              if (currentUserBooking == null) return const SizedBox.shrink();
